...
public class JoinMapActivity extends AppCompatActivity implements OnMapReadyCallback, ActivityCompat.OnRequestPermissionsResultCallback {

    private Handler mHandler;
    private Runnable mRunnable;
    private GoogleMap mMap;
    private Marker currentMarker = null;
    private String promiseId; // promiseId 변수 추가
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.join_gps);
        // 인텐트에서 promiseId와 userId 값을 받아오는 로직
        Intent intent = getIntent();
        if (intent.hasExtra("promiseId")) {
            promiseId = intent.getStringExtra("promiseId");
        }
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(this, REQUIRED_PERMISSION[0])) {
                ...
                Snackbar.make(mLayout, "이 앱을 실행하려면 위치 접근 권한이 필요합니다.", Snackbar.LENGTH_INDEFINITE).setAction("확인", new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        ActivityCompat.requestPermissions(JoinMapActivity.this, REQUIRED_PERMISSION, PERMISSION_REQUEST_CODE);
                    }
                }).show();
            } else {
        ...
        mRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (mCurrentLocation != null) {
                    ...
                    sendLocationDataToServer(latitude, longitude, promiseId, userId);
                }

                // Run the Runnable again after 1 minute
    ...
    LocationCallback locationCallback = new LocationCallback() {
        ...
        public void onLocationResult(@NonNull LocationResult locationResult) {
            ...
            if (locationList.size() > 0) {
                ...
                // sendLocationDataToServer() 메서드 호출
                sendLocationDataToServer(location.getLatitude(), location.getLongitude(), promiseId, userId);
            }
        }
    };
    ...
    private void showDiologForLocationServiceSetting() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(JoinMapActivity.this);
    ...
    private void sendLocationDataToServer(double latitude, double longitude, String promiseId, String userId) {
        ...
        // Create LiveLocationDataRequest object
        LiveLocationDataRequest request = new LiveLocationDataRequest(promiseId, userId, latitude, longitude);

        // Send live location data to the server
        Call<Void> call = apiService.sendLiveLocationData(request);
        call.enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    // Success
                    Toast.makeText(JoinMapActivity.this, "Live location data sent successfully.", Toast.LENGTH_SHORT).show();
                } else {
                    // Failure
                    Toast.makeText(JoinMapActivity.this, "Failed to send live location data.", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(Call<Void> call, Throwable t) {
                // Error
                Toast.makeText(JoinMapActivity.this, "Error sending live location data.", Toast.LENGTH_SHORT).show();
            }
        });
    }
}